...
lib.callback.register('qbx_garages:server:spawnVehicle', function (source, vehicleId, garageName, accessPointIndex)
    ...
    if garageType == GarageType.PARKING then
        if not CanAccessParkingSpot(citizenid, garageName, accessPointIndex) then
        ...
        filter.garage = garageName..':'..accessPointIndex
        filter.state = VehicleState.GARAGED

        playerVehicle = exports.qbx_vehicles:GetPlayerVehicle(vehicleId, filter)
